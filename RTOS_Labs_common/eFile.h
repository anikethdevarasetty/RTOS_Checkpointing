...
#define BLOCK_SIZE 512 //Size of memory blocks on SD cards
#define MAX_BLOCKS 200 //Do not expect to have to use more than these many blocks
#define ENTRY_SIZE 16 //Size of directory entries
...
typedef union{
    dir_entry_t table[BLOCK_SIZE/ENTRY_SIZE];
    uint8_t info[BLOCK_SIZE];
} Directory_u;


/*
	Each block other than the directory block will have meta data-
	The first 4 bytes will be the pointer to the next block in file (null will mean EOF)
	Next 2 bytes will be uint16_t flag telling whether the block is in use or not
	Last 2 bytes will be index of last byte written to in the file
*/
struct block_struct{
		uint32_t next_block;
		uint16_t in_use;
		uint16_t last_write_idx;
		uint8_t info[BLOCK_SIZE-HEADER_SIZE];
};
typedef struct block_struct block_struct_t;

typedef union{
    block_struct_t structured;
    uint8_t whole_block[BLOCK_SIZE];
} Block_u;
 ...
 * @brief  Format the disk
 */
int eFile_Write(const uint32_t data);  
